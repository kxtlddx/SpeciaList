...
const About = () => {
    ...
    const handleClick = (direction) => {
        navigate(`/about/${direction}`)
    }
    
    return (
      <div className='grid grid-cols-4 gap-16 bg-grey-dark px-4 desktop:px-[85px] tablet:px-[39px] pt-16 items-center justify-center pb-16'>
        {directions.map(direction => (
          <div key={direction.title} className='flex flex-col items-center space-y-4'>
            <img src={direction.image} alt={direction.title} class="w-[200px] h-[200px]" onClick={() => handleClick(direction.title)} />
